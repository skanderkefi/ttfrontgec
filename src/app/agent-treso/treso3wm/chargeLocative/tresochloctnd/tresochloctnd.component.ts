...
@Component({
  selector: 'app-tresochloctnd',
  templateUrl: './tresochloctnd.component.html',
  styleUrls: ['./tresochloctnd.component.css']
})
export class TresochloctndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE LOCATIVE - TND" && data[i].status=="validatedFromAp")

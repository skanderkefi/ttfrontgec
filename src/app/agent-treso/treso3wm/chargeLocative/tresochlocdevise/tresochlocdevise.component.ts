...
@Component({
  selector: 'app-tresochlocdevise',
  templateUrl: './tresochlocdevise.component.html',
  styleUrls: ['./tresochlocdevise.component.css']
})
export class TresochlocdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE LOCATIVE - DEVISE" && data[i].status=="validatedFromAp")

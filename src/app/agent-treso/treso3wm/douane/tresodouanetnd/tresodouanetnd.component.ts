...
@Component({
  selector: 'app-tresodouanetnd',
  templateUrl: './tresodouanetnd.component.html',
  styleUrls: ['./tresodouanetnd.component.css']
})
export class TresodouanetndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM DOUANE TND" && data[i].status=="validatedFromAp")

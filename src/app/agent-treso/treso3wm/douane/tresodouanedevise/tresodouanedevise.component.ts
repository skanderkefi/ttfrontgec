...
@Component({
  selector: 'app-tresodouanedevise',
  templateUrl: './tresodouanedevise.component.html',
  styleUrls: ['./tresodouanedevise.component.css']
})
export class TresodouanedeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM DOUANE DEVISE" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresosotdevise',
  templateUrl: './tresosotdevise.component.html',
  styleUrls: ['./tresosotdevise.component.css']
})
export class TresosotdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM DEVISE SOTETEL" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresoancedevise',
  templateUrl: './tresoancedevise.component.html',
  styleUrls: ['./tresoancedevise.component.css']
})
export class TresoancedeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OACA DEVISE" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresocerttnd',
  templateUrl: './tresocerttnd.component.html',
  styleUrls: ['./tresocerttnd.component.css']
})
export class TresocerttndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM TND CERT" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresooacadevise',
  templateUrl: './tresooacadevise.component.html',
  styleUrls: ['./tresooacadevise.component.css']
})
export class TresooacadeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM OACA DEVISE" && data[i].status=="validatedFromAp")

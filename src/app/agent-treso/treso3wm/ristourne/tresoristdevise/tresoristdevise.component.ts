...
@Component({
  selector: 'app-tresoristdevise',
  templateUrl: './tresoristdevise.component.html',
  styleUrls: ['./tresoristdevise.component.css']
})
export class TresoristdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM RISTOURNE DEVISE" && data[i].status=="validatedFromAp")

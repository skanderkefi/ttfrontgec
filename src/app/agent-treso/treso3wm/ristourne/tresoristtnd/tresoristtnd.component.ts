...
@Component({
  selector: 'app-tresoristtnd',
  templateUrl: './tresoristtnd.component.html',
  styleUrls: ['./tresoristtnd.component.css']
})
export class TresoristtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OACA TND" && data[i].status=="validatedFromAp")

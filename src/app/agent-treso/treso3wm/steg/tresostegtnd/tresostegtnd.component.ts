...
@Component({
  selector: 'app-tresostegtnd',
  templateUrl: './tresostegtnd.component.html',
  styleUrls: ['./tresostegtnd.component.css']
})
export class TresostegtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM STEG TND" && data[i].status=="validatedFromAp")

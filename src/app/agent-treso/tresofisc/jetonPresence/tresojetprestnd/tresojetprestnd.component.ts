...
@Component({
  selector: 'app-tresojetprestnd',
  templateUrl: './tresojetprestnd.component.html',
  styleUrls: ['./tresojetprestnd.component.css']
})
export class TresojetprestndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="JETON DE PRÉSENCE TND" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresojetpresdevise',
  templateUrl: './tresojetpresdevise.component.html',
  styleUrls: ['./tresojetpresdevise.component.css']
})
export class TresojetpresdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="JETON DE PRÉSENCE DEVISE" && data[i].status=="validatedFromAp")

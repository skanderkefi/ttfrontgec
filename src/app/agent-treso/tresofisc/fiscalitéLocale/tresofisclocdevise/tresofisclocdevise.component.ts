...
@Component({
  selector: 'app-tresofisclocdevise',
  templateUrl: './tresofisclocdevise.component.html',
  styleUrls: ['./tresofisclocdevise.component.css']
})
export class TresofisclocdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FISCALITÉ LOCALE DEVISE" && data[i].status=="validatedFromAp")

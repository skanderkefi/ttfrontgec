...
@Component({
  selector: 'app-tresoretsaldevise',
  templateUrl: './tresoretsaldevise.component.html',
  styleUrls: ['./tresoretsaldevise.component.css']
})
export class TresoretsaldeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="RETENUE SALAIRE TND" && data[i].status=="validatedFromAp")

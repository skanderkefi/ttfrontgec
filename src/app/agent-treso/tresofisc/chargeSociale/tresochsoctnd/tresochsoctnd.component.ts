...
@Component({
  selector: 'app-tresochsoctnd',
  templateUrl: './tresochsoctnd.component.html',
  styleUrls: ['./tresochsoctnd.component.css']
})
export class TresochsoctndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE SOCIALE TND" && data[i].status=="validatedFromAp")

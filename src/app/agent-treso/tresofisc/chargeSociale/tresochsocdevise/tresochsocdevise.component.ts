...
@Component({
  selector: 'app-tresochsocdevise',
  templateUrl: './tresochsocdevise.component.html',
  styleUrls: ['./tresochsocdevise.component.css']
})
export class TresochsocdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE SOCIALE DEVISE" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresopittnd',
  templateUrl: './tresopittnd.component.html',
  styleUrls: ['./tresopittnd.component.css']
})
export class TresopittndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT IMPOT ET TAXES - TND" && data[i].status=="validatedFromAp")

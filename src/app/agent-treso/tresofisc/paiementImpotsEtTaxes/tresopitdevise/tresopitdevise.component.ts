...
@Component({
  selector: 'app-tresopitdevise',
  templateUrl: './tresopitdevise.component.html',
  styleUrls: ['./tresopitdevise.component.css']
})
export class TresopitdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT IMPOT ET TAXES - DEVISE" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresopeatnd',
  templateUrl: './tresopeatnd.component.html',
  styleUrls: ['./tresopeatnd.component.css']
})
export class TresopeatndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT ET AUTRES TND" && data[i].status=="validatedFromAp")

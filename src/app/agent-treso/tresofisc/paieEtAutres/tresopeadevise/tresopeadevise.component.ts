...
@Component({
  selector: 'app-tresopeadevise',
  templateUrl: './tresopeadevise.component.html',
  styleUrls: ['./tresopeadevise.component.css']
})
export class TresopeadeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT ET AUTRES DEVISE" && data[i].status=="validatedFromAp")

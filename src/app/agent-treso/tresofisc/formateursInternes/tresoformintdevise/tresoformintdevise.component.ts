...
@Component({
  selector: 'app-tresoformintdevise',
  templateUrl: './tresoformintdevise.component.html',
  styleUrls: ['./tresoformintdevise.component.css']
})
export class TresoformintdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FORMATEURS INTERNES TND" && data[i].status=="validatedFromAp")

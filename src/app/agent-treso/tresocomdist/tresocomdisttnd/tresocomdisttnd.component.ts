...
@Component({
  selector: 'app-tresocomdisttnd',
  templateUrl: './tresocomdisttnd.component.html',
  styleUrls: ['./tresocomdisttnd.component.css']
})
export class TresocomdisttndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureComdist().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="COMDIST TND" && data[i].status=="validatedFromAp")

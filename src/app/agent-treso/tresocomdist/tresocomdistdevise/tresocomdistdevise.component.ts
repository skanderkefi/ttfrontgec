...
@Component({
  selector: 'app-tresocomdistdevise',
  templateUrl: './tresocomdistdevise.component.html',
  styleUrls: ['./tresocomdistdevise.component.css']
})
export class TresocomdistdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureComdist().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="COMDIST DEVISE" && data[i].status=="validatedFromAp")

...
@Component({
  selector: 'app-tresooperdevise',
  templateUrl: './tresooperdevise.component.html',
  styleUrls: ['./tresooperdevise.component.css']
})
export class TresooperdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureOp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OPERATEUR DEVISE" && data[i].status=="validatedFromAp")

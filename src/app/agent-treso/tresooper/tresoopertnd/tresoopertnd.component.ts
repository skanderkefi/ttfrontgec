...
@Component({
  selector: 'app-tresoopertnd',
  templateUrl: './tresoopertnd.component.html',
  styleUrls: ['./tresoopertnd.component.css']
})
export class TresoopertndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureOp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM TND SOTETEL" && data[i].status=="validatedFromAp")

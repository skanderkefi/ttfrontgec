...
@Component({
  selector: 'app-tresofdtnd',
  templateUrl: './tresofdtnd.component.html',
  styleUrls: ['./tresofdtnd.component.css']
})
export class TresofdtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacturefd().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FOND DE ROULEMENT TND" && data[i].status=="validatedFromAp")

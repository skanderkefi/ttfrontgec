...
@Component({
  selector: 'app-tresofddevise',
  templateUrl: './tresofddevise.component.html',
  styleUrls: ['./tresofddevise.component.css']
})
export class TresofddeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacturefd().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FOND DE ROULEMENT DEVISE" && data[i].status=="validatedFromAp")

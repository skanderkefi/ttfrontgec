...
@Component({
  selector: 'app-tresoventdirdevise',
  templateUrl: './tresoventdirdevise.component.html',
  styleUrls: ['./tresoventdirdevise.component.css']
})
export class TresoventdirdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="VENTILATION DIRECTE DEVISE" && data[i].status=="validatedFromAp")

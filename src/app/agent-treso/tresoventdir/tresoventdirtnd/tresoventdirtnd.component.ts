...
@Component({
  selector: 'app-tresoventdirtnd',
  templateUrl: './tresoventdirtnd.component.html',
  styleUrls: ['./tresoventdirtnd.component.css']
})
export class TresoventdirtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="VENTILATION DIRECTE TND" && data[i].status=="validatedFromAp")

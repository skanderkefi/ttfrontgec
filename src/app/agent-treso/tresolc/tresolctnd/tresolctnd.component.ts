...
@Component({
  selector: 'app-tresolctnd',
  templateUrl: './tresolctnd.component.html',
  styleUrls: ['./tresolctnd.component.css']
})
export class TresolctndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureLettre().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="LETTRE DE CRÉDIT TND" && data[i].status=="validatedFromAp")

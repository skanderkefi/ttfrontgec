...
@Component({
  selector: 'app-tresolcdevise',
  templateUrl: './tresolcdevise.component.html',
  styleUrls: ['./tresolcdevise.component.css']
})
export class TresolcdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureLettre().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="LETTRE CRÉDIT DEVISE" && data[i].status=="validatedFromAp")

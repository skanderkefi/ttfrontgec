...
@Component({
  selector: 'app-tresofindevise',
  templateUrl: './tresofindevise.component.html',
  styleUrls: ['./tresofindevise.component.css']
})
export class TresofindeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFin().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FINANCEMENT DEVISE" && data[i].status=="validatedFromAp")

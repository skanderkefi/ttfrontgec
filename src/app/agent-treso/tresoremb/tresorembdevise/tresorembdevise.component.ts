...
@Component({
  selector: 'app-tresorembdevise',
  templateUrl: './tresorembdevise.component.html',
  styleUrls: ['./tresorembdevise.component.css']
})
export class TresorembdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFdRemp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="REMBOURSEMENT DEVISE" && data[i].status=="validatedFromAp")

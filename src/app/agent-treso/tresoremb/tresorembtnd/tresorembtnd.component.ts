...
@Component({
  selector: 'app-tresorembtnd',
  templateUrl: './tresorembtnd.component.html',
  styleUrls: ['./tresorembtnd.component.css']
})
export class TresorembtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureFdRemp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="REMBOURSEMENT TND" && data[i].status=="validatedFromAp")

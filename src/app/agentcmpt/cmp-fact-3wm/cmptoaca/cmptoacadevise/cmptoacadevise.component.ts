...
@Component({
  selector: 'app-cmptoacadevise',
  templateUrl: './cmptoacadevise.component.html',
  styleUrls: ['./cmptoacadevise.component.css']
})
export class CmptoacadeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM STEG DEVISE" && data[i].status=="rejectedFromTreso")
    ...
    factureValidateModal(facture){
      ...
      renvoyerFacture(factureToResend){

        factureToResend.status="validatedFromAp";
        factureToResend.dossier="3WM STEG DEVISE";
         this.dataService.updateFacture3wm(factureToResend).subscribe((msg: any[])=>{
           console.log(msg);
         });

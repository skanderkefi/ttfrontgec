...
@Component({
  selector: 'app-cmpopdevise',
  templateUrl: './cmpopdevise.component.html',
  styleUrls: ['./cmpopdevise.component.css']
})
export class CmpopdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureOp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OPERATEUR DEVISE" && data[i].status=="sent")
    ...
    factureValidateModal(facture){
      ...
      convertToPdf() {
        ...
        rejectFacture(){
          ...
          this.dataService.updateFactureOp(this.factureToReject).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }
        ...
        validateFacture(){
          ...
          this.dataService.updateFactureOp(this.factureToValidate).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }

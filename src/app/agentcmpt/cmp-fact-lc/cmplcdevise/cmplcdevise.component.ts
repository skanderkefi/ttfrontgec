...
@Component({
  selector: 'app-cmplcdevise',
  templateUrl: './cmplcdevise.component.html',
  styleUrls: ['./cmplcdevise.component.css']
})
export class CmplcdeviseComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureLettre().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="LETTRE CRÉDIT DEVISE" && data[i].status=="sent")
    ...
    factureValidateModal(facture){
      ...
      convertToPdf() {
        ...
        rejectFacture(){
          ...
          this.dataService.updateFactureLettre(this.factureToReject).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }
        ...
        validateFacture(){
          ...
          this.dataService.updateFactureLettre(this.factureToValidate).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }

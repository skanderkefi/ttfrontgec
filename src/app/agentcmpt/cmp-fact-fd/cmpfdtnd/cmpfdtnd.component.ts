...
@Component({
  selector: 'app-cmpfdtnd',
  templateUrl: './cmpfdtnd.component.html',
  styleUrls: ['./cmpfdtnd.component.css']
})
export class CmpfdtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacturefd().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FOND DE ROULEMENT TND" && data[i].status=="sent")
    ...
    factureValidateModal(facture){
      ...
      convertToPdf() {
        ...
        rejectFacture(){
          ...
          this.dataService.updateFacturefd(this.factureToReject).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }
        ...
        validateFacture(){
          ...
          this.dataService.updateFacturefd(this.factureToValidate).subscribe((msg: any[])=>{
            console.log(msg);
          }) 
          // location.reload();
        }

...
@Component({
  selector: 'app-certdeviseenv',
  templateUrl: './certdeviseenv.component.html',
  styleUrls: ['../../../../facture.css']
})
export class CertdeviseenvComponent implements OnInit {
  ...
  myTexts: IMultiSelectTexts = {
   ...
   "devise": "","direction":"","dossier": "3WM DEVISE CERT" ,"factname":"","fournisseur":"",
   ...
   showFacture(){
     this.dataService.showFacture3wm().subscribe((data: any[])=>{
       ...
       for(let i=0; i<data.length; i++){
         if(data[i].dossier=="3WM DEVISE CERT" && data[i].status=="sent" )

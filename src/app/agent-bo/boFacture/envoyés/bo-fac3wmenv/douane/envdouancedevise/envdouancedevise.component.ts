...
@Component({
  selector: 'app-envdouancedevise',
  templateUrl: './envdouancedevise.component.html',
  styleUrls: ['./envdouancedevise.component.css']
})
export class EnvdouancedeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "DOUANE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="DOUANE - DEVISE" && data[i].status=="sent")

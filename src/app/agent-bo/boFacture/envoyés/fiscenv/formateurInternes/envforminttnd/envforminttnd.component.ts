...
@Component({
  selector: 'app-envforminttnd',
  templateUrl: './envforminttnd.component.html', 
  styleUrls: ['./envforminttnd.component.css']
})
export class EnvforminttndComponent implements OnInit {
  ...
  "devise": "TND", "dossier": "FORMATEURS INTERNES - TND" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FORMATEURS INTERNES - TND" && data[i].status=="sent")

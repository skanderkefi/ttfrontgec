...
@Component({
  selector: 'app-envretsursaldevise',
  templateUrl: './envretsursaldevise.component.html',
  styleUrls: ['./envretsursaldevise.component.css']
})
export class EnvretsursaldeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "RETENUE SALAIRE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="RETENUE SALAIRE - DEVISE" && data[i].status=="sent")

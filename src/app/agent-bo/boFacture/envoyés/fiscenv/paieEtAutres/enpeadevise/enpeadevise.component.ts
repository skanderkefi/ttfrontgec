...
@Component({
  selector: 'app-enpeadevise',
  templateUrl: './enpeadevise.component.html',
  styleUrls: ['./enpeadevise.component.css']
})
export class EnpeadeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "PAIEMENT ET AUTRES - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT ET AUTRES - DEVISE" && data[i].status=="sent")

...
@Component({
  selector: 'app-envretsursaltnd',
  templateUrl: './envretsursaltnd.component.html',
  styleUrls: ['./envretsursaltnd.component.css']
})
export class EnvretsursaltndComponent implements OnInit {
  ...
  "devise": "TND", "dossier": "RETENUE SALAIRE - TND" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="RETENUE SALAIRE - TND" && data[i].status=="sent")

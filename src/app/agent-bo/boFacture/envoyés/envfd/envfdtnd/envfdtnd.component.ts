...
@Component({
  selector: 'app-envfdtnd',
  templateUrl: './envfdtnd.component.html',
  styleUrls: ['../../../facture.css']
})
export class EnvfdtndComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureComdist().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FOND DE ROULEMENT" && data[i].status=="sent")

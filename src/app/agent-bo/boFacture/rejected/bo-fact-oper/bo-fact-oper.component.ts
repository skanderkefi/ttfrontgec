...
@Component({
  selector: 'app-bo-fact-oper',
  templateUrl: './bo-fact-oper.component.html',
  styleUrls: ['./bo-fact-oper.component.css']
})
export class BoFactOperComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureOp().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OPERATEUR TND" && data[i].status=="rejected")

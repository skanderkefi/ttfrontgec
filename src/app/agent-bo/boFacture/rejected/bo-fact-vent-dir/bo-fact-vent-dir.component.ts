...
@Component({
  selector: 'app-bo-fact-vent-dir',
  templateUrl: './bo-fact-vent-dir.component.html',
  styleUrls: ['./bo-fact-vent-dir.component.css']
})
export class BoFactVentDirComponent implements OnInit {
  ...
  "factname":"","fournisseur":"", "status":"","montant":"","num_fact":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="VENTILATION DIRECTE TND" && data[i].status=="rejected")

...
@Component({
  selector: 'app-rejchsoctnd',
  templateUrl: './rejchsoctnd.component.html',
  styleUrls: ['./rejchsoctnd.component.css']
})
export class RejchsoctndComponent implements OnInit {
  ...
  "devise": "TND", "dossier": "CHARGE SOCIALE TND" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE SOCIALE TND" && data[i].status=="rejected")

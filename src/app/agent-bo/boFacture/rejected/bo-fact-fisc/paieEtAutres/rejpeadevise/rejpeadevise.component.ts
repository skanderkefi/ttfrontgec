...
@Component({
  selector: 'app-rejpeadevise',
  templateUrl: './rejpeadevise.component.html',
  styleUrls: ['./rejpeadevise.component.css']
})
export class RejpeadeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "PAIEMENT ET AUTRE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT ET AUTRE - DEVISE" && data[i].status=="rejected")

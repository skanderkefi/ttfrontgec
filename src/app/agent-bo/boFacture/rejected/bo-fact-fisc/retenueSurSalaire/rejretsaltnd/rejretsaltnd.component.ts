...
@Component({
  selector: 'app-rejretsaltnd',
  templateUrl: './rejretsaltnd.component.html',
  styleUrls: ['./rejretsaltnd.component.css']
})
export class RejretsaltndComponent implements OnInit {
  selectedFile: File;
  ...
  "devise": "TND", "dossier": "RETENUE SUR SALAIRE - TND" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="RETENUE SUR SALAIRE - TND" && data[i].status=="rejected")

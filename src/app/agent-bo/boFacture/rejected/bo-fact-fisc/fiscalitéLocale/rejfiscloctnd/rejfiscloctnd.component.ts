...
@Component({
  selector: 'app-rejfiscloctnd',
  templateUrl: './rejfiscloctnd.component.html',
  styleUrls: ['./rejfiscloctnd.component.css']
})
export class RejfiscloctndComponent implements OnInit {
  ...
  "devise": "TND", "dossier": "FISCALTÉ LOCALE - TND" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="FISCALTÉ LOCALE - TND" && data[i].status=="rejected")

...
@Component({
  selector: 'app-rejpitxdevise',
  templateUrl: './rejpitxdevise.component.html',
  styleUrls: ['./rejpitxdevise.component.css']
})
export class RejpitxdeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "PAIEMENT IMPOTS ET TAXES - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="PAIEMENT IMPOTS ET TAXES - DEVISE" && data[i].status=="rejected")

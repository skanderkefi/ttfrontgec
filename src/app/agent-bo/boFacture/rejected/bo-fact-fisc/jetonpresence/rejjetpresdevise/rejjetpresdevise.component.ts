...
@Component({
  selector: 'app-rejjetpresdevise',
  templateUrl: './rejjetpresdevise.component.html',
  styleUrls: ['./rejjetpresdevise.component.css']
})
export class RejjetpresdeviseComponent implements OnInit {
  ...
  "devise": "", "dossier": "JETON DE PRÉSENCE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFactureFisc().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="JETON DE PRÉSENCE - DEVISE" && data[i].status=="rejected")

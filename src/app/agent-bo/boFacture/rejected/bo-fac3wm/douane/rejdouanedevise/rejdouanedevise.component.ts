...
@Component({
  selector: 'app-rejdouanedevise',
  templateUrl: './rejdouanedevise.component.html',
  styleUrls: ['./rejdouanedevise.component.css']
})
export class RejdouanedeviseComponent implements OnInit {
 ...
 myTexts: IMultiSelectTexts = {
  ...
  "devise": "","direction":"","dossier": "DOUANE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE LOCATIVE - DEVISE" && data[i].status=="rejected" )

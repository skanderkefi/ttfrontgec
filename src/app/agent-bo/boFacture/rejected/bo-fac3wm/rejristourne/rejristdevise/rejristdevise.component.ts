...
@Component({
  selector: 'app-rejristdevise',
  templateUrl: './rejristdevise.component.html',
  styleUrls: ['./rejristdevise.component.css']
})
export class RejristdeviseComponent implements OnInit {
 ...
 myTexts: IMultiSelectTexts = {
  ...
  "devise": "","direction":"","dossier": "RISTOURNE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="RISTOURNE - DEVISE" && data[i].status=="rejected" )

...
@Component({
  selector: 'app-rejchlocdevise',
  templateUrl: './rejchlocdevise.component.html',
  styleUrls: ['./rejchlocdevise.component.css']
})
export class RejchlocdeviseComponent implements OnInit {
 ...
 myTexts: IMultiSelectTexts = {
  ...
  "devise": "","direction":"","dossier": "CHARGE LOCATIVE - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="CHARGE LOCATIVE - CERT" && data[i].status=="rejected" )

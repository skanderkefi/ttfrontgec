...
@Component({
  selector: 'app-bo3wmsteg',
  templateUrl: './bo3wmsteg.component.html',
  styleUrls: ['./bo3wmsteg.component.css']
})
export class Bo3wmstegtndrjcComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier="3WM STEG TND" && data[i].status=="rejected")

...
@Component({
  selector: 'app-rejoacadevise',
  templateUrl: './rejoacadevise.component.html',
  styleUrls: ['./rejoacadevise.component.css']
})
export class RejoacadeviseComponent implements OnInit {
 ...
 myTexts: IMultiSelectTexts = {
  ...
  "devise": "","direction":"","dossier": "OACA - DEVISE" ,"factname":"","fournisseur":"",
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="OACA - DEVISE" && data[i].status=="rejected" )

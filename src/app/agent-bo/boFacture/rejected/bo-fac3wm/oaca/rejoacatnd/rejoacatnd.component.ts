...
@Component({
  selector: 'app-rejoacatnd',
  templateUrl: './rejoacatnd.component.html',
  styleUrls: ['./rejoacatnd.component.css']
})
export class RejoacatndComponent implements OnInit {
   ...
   myTexts: IMultiSelectTexts = {
    ...
    "devise": "TND","direction":"","dossier": "OACA - TND" ,"factname":"","fournisseur":"",
    ...
    showFacture(){
      this.dataService.showFacture3wm().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          if(data[i].dossier=="OACA - TND" && data[i].status=="rejected")

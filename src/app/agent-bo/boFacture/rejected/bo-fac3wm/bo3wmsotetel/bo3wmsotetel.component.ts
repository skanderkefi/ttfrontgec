...
@Component({
  selector: 'app-bo3wmsotetel',
  templateUrl: './bo3wmsotetel.component.html',
  styleUrls: ['./bo3wmsotetel.component.css']
})
export class Bo3wmsotetelComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFacture3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="3WM TND SOTETEL" &&data[i].status=="rejected" )

...
@Component({
  selector: 'app-bo-fact-comdit',
  templateUrl: './bo-fact-comdit.component.html',
  styleUrls: ['./bo-fact-comdit.component.css']
})
export class BoFactComditComponent implements OnInit {
  ...
  showFacture(){
    this.dataService.showFactureComdist().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].dossier=="COMDIST TND" && data[i].status=="rejected")

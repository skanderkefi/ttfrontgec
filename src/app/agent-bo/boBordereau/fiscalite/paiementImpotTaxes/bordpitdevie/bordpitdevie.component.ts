...
@Component({
  selector: 'app-bordpitdevie',
  templateUrl: './bordpitdevie.component.html',
  styleUrls: ['./bordpitdevie.component.css']
})
export class BordpitdevieComponent implements OnInit {
  ...
  "nature":"PAIEMENT IMPOT ET TAXES","natureRaff":"DEVISE","reference":"BOF/PAIEMENT IMPOT ET TAXES-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="PAIEMENT IMPOT ET TAXES" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="PAIEMENT IMPOT ET TAXES - DEVISE"){

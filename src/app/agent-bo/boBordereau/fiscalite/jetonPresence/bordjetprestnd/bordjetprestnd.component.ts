...
@Component({
  selector: 'app-bordjetprestnd',
  templateUrl: './bordjetprestnd.component.html',
  styleUrls: ['./bordjetprestnd.component.css']
})
export class BordjetprestndComponent implements OnInit {
  ...
  "nature":"JETON DE PRÉSENCE","natureRaff":"TND","reference":"BOF/JETON DE PRÉSENCE-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="JETON DE PRÉSENCE" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="JETON DE PRÉSENCE TND"){

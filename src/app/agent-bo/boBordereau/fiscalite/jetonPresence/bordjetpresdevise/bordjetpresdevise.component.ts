...
@Component({
  selector: 'app-bordjetpresdevise',
  templateUrl: './bordjetpresdevise.component.html',
  styleUrls: ['./bordjetpresdevise.component.css']
})
export class BordjetpresdeviseComponent implements OnInit {
  ...
  "nature":"JETON DE PRÉSENCE","natureRaff":"DEVISE","reference":"BOF/JETON DE PRÉSENCE-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="JETON DE PRÉSENCE" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="JETON DE PRÉSENCE DEVISE"){

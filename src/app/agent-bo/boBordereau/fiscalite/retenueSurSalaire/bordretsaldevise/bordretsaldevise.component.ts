...
@Component({
  selector: 'app-bordretsaldevise',
  templateUrl: './bordretsaldevise.component.html',
  styleUrls: ['./bordretsaldevise.component.css']
})
export class BordretsaldeviseComponent implements OnInit {
  ...
  "nature":"RETENUE SALAIRE","natureRaff":"DEVISE","reference":"BOF/RETENUE SALAIRE-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="RETENUE SALAIRE" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="RETENUE SALAIRE DEVISE"){

...
export class BordretsaltndComponent implements OnInit {
  ...
  "nature":"RETENUE SALAIRE","natureRaff":"TND","reference":"BOF/RETENUE SALAIRE-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="RETENUE SALAIRE" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="RETENUE SALAIRE TND"){

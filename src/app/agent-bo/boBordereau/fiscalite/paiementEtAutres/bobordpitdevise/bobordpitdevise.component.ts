...
@Component({
  selector: 'app-bobordpitdevise',
  templateUrl: './bobordpitdevise.component.html',
  styleUrls: ['./bobordpitdevise.component.css']
})
export class BobordpeadeviseComponent implements OnInit {
  ...
  "nature":"PAIEMENT ET AUTRES","natureRaff":"DEVISE","reference":"BOF/PAIEMENT ET AUTRES-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="PAIEMENT ET AUTRES" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="PAIEMENT ET AUTRES DEVISE"){

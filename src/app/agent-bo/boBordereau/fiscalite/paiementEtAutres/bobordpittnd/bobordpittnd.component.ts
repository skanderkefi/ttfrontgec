...
@Component({
  selector: 'app-bobordpittnd',
  templateUrl: './bobordpittnd.component.html',
  styleUrls: ['./bobordpittnd.component.css']
})
export class BobordpeatndComponent implements OnInit {
  ...
  "nature":"PAIEMENT ET AUTRES","natureRaff":"TND","reference":"BOF/PAIEMENT ET AUTRES-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="PAIEMENT ET AUTRES" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="PAIEMENT ET AUTRES TND"){

...
@Component({
  ...
  styleUrls: ['./fiscalite.component.css']
})
export class FiscaliteComponent implements OnInit {
  ...
  "nature":"FISCALTÉ LOCALE","natureRaff":"TND","reference":"BOF/FISCALTÉ LOCALE-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="FISCALTÉ LOCALE" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="FISCALTÉ LOCALE TND"){

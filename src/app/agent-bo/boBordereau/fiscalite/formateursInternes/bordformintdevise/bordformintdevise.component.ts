...
@Component({
  selector: 'app-bordformintdevise',
  templateUrl: './bordformintdevise.component.html',
  styleUrls: ['./bordformintdevise.component.css']
})
export class BordformintdeviseComponent implements OnInit {
  ...
  "nature":"FORMATEURS INTERNES","natureRaff":"DEVISE","reference":"BOF/FORMATEURS INTERNES-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="FORMATEURS INTERNES" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFisc().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="FORMATEURS INTERNES DEVISE"){

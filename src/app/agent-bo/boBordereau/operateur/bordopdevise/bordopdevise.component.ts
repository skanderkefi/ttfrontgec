...
@Component({
  selector: 'app-bordopdevise',
  templateUrl: './bordopdevise.component.html',
  styleUrls: ['./bordopdevise.component.css']
})
export class BordopdeviseComponent implements OnInit {
  ...
  "nature":"OPÉRATEUR","natureRaff":"DEVISE","reference":"BOF/OPÉRATEUR-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="OPÉRATEUR" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureOp().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="OPERATEUR DEVISE"){
...
let sentToAp:string;
  ...
  for (let i=0;i<this.Factures3wmTnd.length;i++){
    ...
    this.dataService.updateFactureOp(this.Factures3wmTnd[i]).subscribe((msg: any[])=>{
      console.log(msg);
    });
  }

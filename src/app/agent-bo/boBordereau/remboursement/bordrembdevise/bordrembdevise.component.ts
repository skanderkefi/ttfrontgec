...
@Component({
  selector: 'app-bordrembdevise',
  templateUrl: './bordrembdevise.component.html',
  styleUrls: ['./bordrembdevise.component.css']
})
export class BordrembdeviseComponent implements OnInit {
  ...
  "nature":"REMBOURSEMENT","natureRaff":"DEVISE","reference":"BOF/REMBOURSEMENT-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="REMBOURSEMENT" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFdRemp().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="REMBOURSEMENT DEVISE"){
...
let sentToAp:string;
  ...
  for (let i=0;i<this.Factures3wmTnd.length;i++){
    ...
    this.dataService.updateFactureFdRemp(this.Factures3wmTnd[i]).subscribe((msg: any[])=>{
      console.log(msg);
    });
  }

...
@Component({
  selector: 'app-bordfindevise',
  templateUrl: './bordfindevise.component.html',
  styleUrls: ['./bordfindevise.component.css']
})
export class BordfindeviseComponent implements OnInit {
  ...
  "nature":"FINANCEMENT","natureRaff":"DEVISE","reference":"BOF/FINANCEMENT-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="FINANCEMENT" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFactureFin().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="FINANCEMENT DEVISE"){
...
let sentToAp:string;
  ...
  for (let i=0;i<this.Factures3wmTnd.length;i++){
    ...
    this.dataService.updateFactureFin(this.Factures3wmTnd[i]).subscribe((msg: any[])=>{
      console.log(msg);
    });
  }

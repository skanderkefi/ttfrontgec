...
@Component({
  selector: 'app-bobordsotdevise',
  templateUrl: './bobordsotdevise.component.html',
  styleUrls: ['./bobordsotdevise.component.css']
})
export class BobordsotdeviseComponent implements OnInit {
  ...
  "nature":"SOTETEL","natureRaff":"DEVISE","reference":"BOF/SOTETEL-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="SOTETEL" && data[i].natureRaff=="DEVISE"){
...
let sentToAp:string;
  ...
  for (let i=0;i<this.Factures3wmTnd.length;i++){
    ...
    this.dataService.updateFacturecmd(this.Factures3wmTnd[i]).subscribe((msg: any[])=>{
      console.log(msg);
    });
  }

...
@Component({
  selector: 'app-ventdir',
  templateUrl: './ventdir.component.html',
  styleUrls: ['../../facture.css']
})
export class VentdirComponent implements OnInit {

  Factures3wmTnd:any[]=[];
  firstn:any;
  Bordereaux:any[];
  Bordereaux3wmTnd:any[]=[];
  bordereauToCreate={"createdAt":Date.now(),"createdBy":"4125","folder":"3WM VENTILATION DIRECTE","id":"",
  "nature":"3WM","natureRaff":"TND","reference":"BOF/3WM-VENTILATION DIRECTE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].folder=="3WM VENTILATION DIRECTE" )

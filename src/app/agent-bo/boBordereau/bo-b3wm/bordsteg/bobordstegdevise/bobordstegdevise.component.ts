...
@Component({
  selector: 'app-bobordstegdevise',
  templateUrl: './bobordstegdevise.component.html',
  styleUrls: ['./bobordstegdevise.component.css']
})
export class BobordstegdeviseComponent implements OnInit {
  ...
  "nature":"STEG","natureRaff":"DEVISE","reference":"BOF/STEG-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="STEG" && data[i].natureRaff=="DEVISE" && data[i].status!="sentToAp"){
...
envoyerBordereau(){
  ...
  //set factures status to "senttoap"
  for (let i=0;i<this.Factures3wmTnd.length;i++){
    this.Factures3wmTnd[i].status="sentToAp";
  ...
  this.bordereauToUpdate.status="sentToAp";
...
console.log("bordereau to update in envoie method notmodal="+this.bordereauToUpdate);
  ...
  showUserModal(bord){
    ...
    console.log(("bord to update"+this.bordereauToUpdate));

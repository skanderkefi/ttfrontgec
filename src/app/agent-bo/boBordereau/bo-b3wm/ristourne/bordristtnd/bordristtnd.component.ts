...
@Component({
  selector: 'app-bordristtnd',
  templateUrl: './bordristtnd.component.html',
  styleUrls: ['./bordristtnd.component.css']
})
export class BordristtndComponent implements OnInit {
  ...
  "nature":"RISTOURNE","natureRaff":"TND","reference":"BOF/RISTOURNE-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="RISTOURNE" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFacture3wm().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="3WM RISTOURNE TND"){

...
@Component({
  selector: 'app-bordlc',
  templateUrl: './bordlc.component.html',
  styleUrls: ['../../facture.css']
})
export class BordlcComponent implements OnInit {
  ...
  "nature":"LETTRE CRÉDIT","natureRaff":"TND","reference":"BOF/LC-TND/","sentAt":"","sentBy":"",
  "status":"","updatedAt":Date.now(),"toAp":""};
  bordereauToUpdate={"createdAt":Date.now(),"createdBy":"4125","folder":"","id":"",
  "nature":"LETTRE CRÉDIT","natureRaff":"TND","reference":"BOF/LC-DEVISE/","sentAt":"","sentBy":"",
  "status":"","updatedAt":Date.now(),"toAp":""};
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="LETTRE CRÉDIT" && data[i].natureRaff=="TND"){

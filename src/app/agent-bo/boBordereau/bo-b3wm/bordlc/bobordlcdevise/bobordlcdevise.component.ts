...
@Component({
  selector: 'app-bobordlcdevise',
  templateUrl: './bobordlcdevise.component.html',
  styleUrls: ['./bobordlcdevise.component.css']
})
export class BobordlcdeviseComponent implements OnInit {
  ...
  "nature":"LETTRE CRÉDIT","natureRaff":"DEVISE","reference":"BOF/LC-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="LETTRE CRÉDIT" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFacture3wm().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="LETTRE DE CRÉDIT DEVISE"){

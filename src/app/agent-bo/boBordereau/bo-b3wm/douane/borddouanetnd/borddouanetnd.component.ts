...
@Component({
  selector: 'app-borddouanetnd',
  templateUrl: './borddouanetnd.component.html',
  styleUrls: ['./borddouanetnd.component.css']
})
export class BorddouanetndComponent implements OnInit {
  ...
  "nature":"DOUANE","natureRaff":"TND","reference":"BOF/DOUANE-TND/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="DOUANE" && data[i].natureRaff=="TND"){
    ...
    showFacture(){
      this.dataService.showFacture3wm().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="3WM DOUANE TND"){

...
@Component({
  selector: 'app-bobordcertdevise',
  templateUrl: './bobordcertdevise.component.html',
  styleUrls: ['./bobordcertdevise.component.css']
})
export class BobordcertdeviseComponent implements OnInit {
  ...
  "nature":"CERT","natureRaff":"DEVISE","reference":"BOF/CERT-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].nature=="CERT" && data[i].natureRaff=="DEVISE"){
    ...
    showFacture(){
      this.dataService.showFacture3wm().subscribe((data: any[])=>{
        ...
        for(let i=0; i<data.length; i++){
          ...
          if(data[i].dossier=="3WM TND CERT"){

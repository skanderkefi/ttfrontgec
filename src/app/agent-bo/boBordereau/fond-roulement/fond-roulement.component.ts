...
@Component({
  selector: 'app-fond-roulement',
  templateUrl: './fond-roulement.component.html',
  styleUrls: ['./fond-roulement.component.css']
})
export class FondRoulementComponent implements OnInit {

  Factures3wmTnd:any[]=[];
  firstn:any;
  Bordereaux:any[];
  Bordereaux3wmTnd:any[]=[];
  bordereauToCreate={"createdAt":Date.now(),"createdBy":"4125","folder":"FOND DE ROULEMENT TND","id":"",
  "nature":"FOND DE ROULEMENT","natureRaff":"TND","reference":"BOF/FOND DE ROULEMENT/","sentAt":"","sentBy":"",
  "status":"en cours","updatedAt":"","toAp":""};

  bordereauToUpdate={"createdAt":Date.now(),"createdBy":"4125","folder":"FOND DE ROULEMENT TND","id":"",
  "nature":"FOND DE ROULEMENT","natureRaff":"TND","reference":"BOF/FOND DE ROULEMENT/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].folder=="FOND DE ROULEMENT TND" )
        this.Bordereaux3wmTnd[i]=data[i];
      }
      for(let i=0; i<this.Bordereaux3wmTnd.length; i++){
        if(this.Bordereaux3wmTnd[i].folder!="FOND DE ROULEMENT TND")
...
showFacture(){
  this.dataService.showFacturefd().subscribe((data: any[])=>{
    ...
    for(let i=0; i<data.length; i++){
      ...
      if(data[i].dossier=="FOND ROULEMENT TND"){

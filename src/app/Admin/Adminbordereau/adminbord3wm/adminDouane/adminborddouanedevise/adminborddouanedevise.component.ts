...
@Component({
  selector: 'app-adminborddouanedevise',
  templateUrl: './adminborddouanedevise.component.html',
  styleUrls: ['./adminborddouanedevise.component.css']
})
export class AdminborddouanedeviseComponent implements OnInit {

  Factures3wmTnd:any[]=[];
  firstn:any;
  Bordereaux:any[];
  Bordereaux3wmTnd:any[]=[];
  bordereauToCreate={"createdAt":Date.now(),"createdBy":"4125","folder":"3WM DOUANE DEVISE","id":"",
  "nature":"3WM","natureRaff":"DEVISE","reference":"BOF/3WM-DOUANE-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].folder=="3WM DOUANE DEVISE" )

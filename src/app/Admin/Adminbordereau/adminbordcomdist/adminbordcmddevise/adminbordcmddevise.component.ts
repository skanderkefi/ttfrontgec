...
@Component({
  selector: 'app-adminbordcmddevise',
  templateUrl: './adminbordcmddevise.component.html',
...
export class AdminbordcmddeviseComponent implements OnInit {
  ...
  bordereauToCreate={"createdAt":Date.now(),"createdBy":"4125","folder":"COMMISSION ET DISTRIBUTION DEVISE","id":"",
  "nature":"COMMISSION ET DISTRIBUTION","natureRaff":"DEVISE","reference":"BOF/COMMISSION-DISTRIBUTION-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].folder=="COMMISSION ET DISTRIBUTION DEVISE" )

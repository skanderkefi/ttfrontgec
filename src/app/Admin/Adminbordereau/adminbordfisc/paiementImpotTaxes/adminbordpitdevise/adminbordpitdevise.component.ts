...
@Component({
  selector: 'app-adminbordpitdevise',
  templateUrl: './adminbordpitdevise.component.html',
  styleUrls: ['./adminbordpitdevise.component.css']
})
export class AdminbordpitdeviseComponent implements OnInit {

  Factures3wmTnd:any[]=[];
  firstn:any;
  Bordereaux:any[];
  Bordereaux3wmTnd:any[]=[];
  bordereauToCreate={"createdAt":Date.now(),"createdBy":"4125","folder":"FISCALTÉ PAIEMENT IMPÔTS ET TAXES - DEVISE","id":"",
  "nature":"FISCALTÉ","natureRaff":"DEVISE","reference":"BOF/FISCALTÉ-PAIEMENT IMPÔTS ET TAXES-DEVISE/","sentAt":"","sentBy":"",
  ...
  showBordereaux(){
    this.dataService.showBordereau3wm().subscribe((data: any[])=>{
      ...
      for(let i=0; i<data.length; i++){
        if(data[i].folder=="FISCALTÉ PAIEMENT IMPÔTS ET TAXES - DEVISE" )
